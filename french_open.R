# This script runs the statistical analyses for a paper on athletes, gender, and risk for the French Open only
...
data <- read.csv(paste(directory, 'french.csv', sep = ''), fileEncoding = 'UTF-8-BOM')
...
mod6 <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Doubles + Qualifier + Ranking_S
            + Titles + Majors + French_Open, family = binomial(link = logit), data = data)
...
mod7 <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Doubles + Qualifier + Ranking_S
            + Titles + Majors_Bi + French_Open, family = binomial(link = logit), data = data)

mod8 <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Doubles + Qualifier + Ranking_S
            + Titles + Majors + French_Open_Bi, family = binomial(link = logit), data = data)

mod9 <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Doubles + Qualifier + Ranking_S
            + Titles + Majors_Bi + French_Open_Bi, family = binomial(link = logit), data = data)

# Running models 6-9 with Doubles dropped to prevent the error of quasi-complete data separation

mod6d <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Qualifier + Ranking_S
            + Titles + Majors + French_Open, family = binomial(link = logit), data = data)

mod7d <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Qualifier + Ranking_S
            + Titles + Majors_Bi + French_Open, family = binomial(link = logit), data = data)

mod8d <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Qualifier + Ranking_S
            + Titles + Majors + French_Open_Bi, family = binomial(link = logit), data = data)

mod9d <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Qualifier + Ranking_S
            + Titles + Majors_Bi + French_Open_Bi, family = binomial(link = logit), data = data)
...
submod6 <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Doubles + Qualifier + Ranking_S
            + Titles + Majors + French_Open, family = binomial(link = logit), data = subdata)

# Models 7-9 are Model 6 with alternative specifications for Majors and French_Open to serve as robustness checks

submod7 <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Doubles + Qualifier + Ranking_S
            + Titles + Majors_Bi + French_Open, family = binomial(link = logit), data = subdata)

submod8 <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Doubles + Qualifier + Ranking_S
            + Titles + Majors + French_Open_Bi, family = binomial(link = logit), data = subdata)

submod9 <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Doubles + Qualifier + Ranking_S
            + Titles + Majors_Bi + French_Open_Bi, family = binomial(link = logit), data = subdata)

# Running models 6-9 with Doubles dropped to prevent the error of quasi-complete data separation

submod6d <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Qualifier + Ranking_S
             + Titles + Majors + French_Open, family = binomial(link = logit), data = subdata)

submod7d <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Qualifier + Ranking_S
             + Titles + Majors_Bi + French_Open, family = binomial(link = logit), data = subdata)

submod8d <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Qualifier + Ranking_S
             + Titles + Majors + French_Open_Bi, family = binomial(link = logit), data = subdata)

submod9d <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Qualifier + Ranking_S
             + Titles + Majors_Bi + French_Open_Bi, family = binomial(link = logit), data = subdata)
...
cov <- vcovHC(mod6d, type = 'HC0')
rse6d <- sqrt(diag(cov))

cov <- vcovHC(mod7d, type = 'HC0')
rse7d <- sqrt(diag(cov))

cov <- vcovHC(mod8d, type = 'HC0')
rse8d <- sqrt(diag(cov))

cov <- vcovHC(mod9d, type = 'HC0')
rse9d <- sqrt(diag(cov))
...
cov <- vcovHC(submod6d, type = 'HC0')
subrse6d <- sqrt(diag(cov))

cov <- vcovHC(submod7d, type = 'HC0')
subrse7d <- sqrt(diag(cov))

cov <- vcovHC(submod8d, type = 'HC0')
subrse8d <- sqrt(diag(cov))

cov <- vcovHC(submod9d, type = 'HC0')
subrse9d <- sqrt(diag(cov))

# Viewing regression results and saving to file

stargazer(mod1, mod2, mod3, mod4, mod5, mod6, mod7, mod8, mod9, mod6d, mod7d, mod8d, mod9d, type = 'text',
          se = list(rse1, rse2, rse3, rse4, rse5, rse6, rse7, rse8, rse9, rse6d, rse7d, rse8d, rse9d))
stargazer(submod1, submod2, submod3, submod4, submod5, submod6, submod7, submod8, submod9, submod6d, submod7d, submod8d, submod9d, type = 'text',
          se = list(subrse1, subrse2, subrse3, subrse4, subrse5, subrse6, subrse7, subrse8, subrse9, subrse6d, subrse7d, subrse8d, subrse9d))

write.csv(stargazer(mod1, mod2, mod3, mod4, mod5, mod6, mod7, mod8, mod9, type = 'text',
                    se = list(rse1, rse2, rse3, rse4, rse5, rse6, rse7, rse8, rse9)),
          paste(directory, 'results_all_french.txt', sep = ''), row.names = FALSE)
write.csv(stargazer(submod1, submod2, submod3, submod4, submod5, submod6, submod7, submod8, submod9, type = 'text',
          ...
          paste(directory, 'results_lucky_french.txt', sep = ''), row.names = FALSE)

write.csv(stargazer(mod1, mod2, mod3, mod4, mod5, mod6, mod7, mod8, mod9,
                    se = list(rse1, rse2, rse3, rse4, rse5, rse6, rse7, rse8, rse9)),
          paste(directory, 'results_all_french_tex.txt', sep = ''), row.names = FALSE)
write.csv(stargazer(submod1, submod2, submod3, submod4, submod5, submod6, submod7, submod8, submod9,
                    se = list(subrse1, subrse2, subrse3, subrse4, subrse5, subrse6, subrse7, subrse8, subrse9)),
          paste(directory, 'results_lucky_french_tex.txt', sep = ''), row.names = FALSE)

write.csv(stargazer(mod1, mod2, mod3, mod4, mod5, mod6, mod7, mod8, mod9, mod6d, mod7d, mod8d, mod9d, type = 'text',
                    se = list(rse1, rse2, rse3, rse4, rse5, rse6, rse7, rse8, rse9, rse6d, rse7d, rse8d, rse9d)),
          paste(directory, 'results_all_french_2.txt', sep = ''), row.names = FALSE)
write.csv(stargazer(submod1, submod2, submod3, submod4, submod5, submod6, submod7, submod8, submod9, submod6d, submod7d, submod8d, submod9d, type = 'text',
                    se = list(subrse1, subrse2, subrse3, subrse4, subrse5, subrse6, subrse7, subrse8, subrse9, subrse6d, subrse7d, subrse8d, subrse9d)),
          paste(directory, 'results_lucky_french_2.txt', sep = ''), row.names = FALSE)

write.csv(stargazer(mod1, mod2, mod3, mod4, mod5, mod6, mod7, mod8, mod9, mod6d, mod7d, mod8d, mod9d,
                    se = list(rse1, rse2, rse3, rse4, rse5, rse6, rse7, rse8, rse9, rse6d, rse7d, rse8d, rse9d)),
          paste(directory, 'results_all_french_tex_2.txt', sep = ''), row.names = FALSE)
write.csv(stargazer(submod1, submod2, submod3, submod4, submod5, submod6, submod7, submod8, submod9, submod6d, submod7d, submod8d, submod9d,
                    se = list(subrse1, subrse2, subrse3, subrse4, subrse5, subrse6, subrse7, subrse8, subrse9, subrse6d, subrse7d, subrse8d, subrse9d)),
          paste(directory, 'results_lucky_french_tex_2.txt', sep = ''), row.names = FALSE)

stargazer(mod1, mod2, mod3, mod4, mod5, mod6, mod7, mod8, mod9, type = 'text',
          se = list(rse1, rse2, rse3, rse4, rse5, rse6, rse7, rse8, rse9))
