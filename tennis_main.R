...
omod6f <- lm(Competed ~ Age + Age2 + log(Winnings + 1) + log(Winnings_20 + 1)
             + Doubles + Qualifier + Ranking_S + Titles + Majors_Bi + Current_Bi
             + factor(Tournament)+factor(Country) + Prev_Tourn_Comp,
             data = subdataf)
...
omod6m <- lm(Competed ~ Age + Age2 + log(Winnings + 1) + log(Winnings_20 + 1)
             + Doubles + Qualifier + Ranking_S + Titles + Majors_Bi + Current_Bi
             + factor(Tournament)+factor(Country) + Prev_Tourn_Comp,
             data = subdatam)
...
write.csv(stargazer(amod2, amod3, amod4, amod5, amod6, amod7,
          ...
          paste(directory, 'results/groups/results_lucky_full_season_w_followers_10s_OLS_tex.txt', sep = ''), row.names = FALSE)

write.csv(stargazer(a2mod2, a2mod3, a2mod4, a2mod5, a2mod6, a2mod7,
                    se = list(a22, a23, a24, a25, a26, a27), omit = c('Country', 'Tournament')),
          paste(directory, 'results/groups/results_lucky_full_season_w_followers_20s_OLS_tex.txt', sep = ''), row.names = FALSE)

write.csv(stargazer(a3mod2, a3mod3, a3mod4, a3mod5, a3mod6, a3mod7,
                    se = list(a32, a33, a34, a35, a36,a37), omit = c('Country', 'Tournament')),
          paste(directory, 'results/groups/results_lucky_full_season_w_followers_30s_OLS_tex.txt', sep = ''), row.names = FALSE)
...
write.csv(stargazer(umod2, umod3, umod4, umod5, umod6, umod7,
          ...
          paste(directory, 'results/us/results_lucky_full_season_w_followers_OLS_tex.txt', sep = ''), row.names = FALSE)

write.csv(stargazer(imod2, imod3, imod4, imod5, imod6, imod7,
                    se = list(i2, i3, i4, i5, i6, i7), omit = c('Country', 'Tournament')),
          paste(directory, 'results/italian/results_lucky_full_season_w_followers_OLS_tex.txt', sep = ''), row.names = FALSE)

write.csv(stargazer(fmod2, fmod3, fmod4, fmod5, fmod6, fmod7,
                    se = list(f2, f3, f4, f5, f6, f7), omit = c('Country', 'Tournament')),
          paste(directory, 'results/french/results_lucky_full_season_w_followers_OLS_tex.txt', sep = ''), row.names = FALSE)
...
write.csv(stargazer(amod2, amod3, amod4, amod5, amod6, amod7,
                    se = list(a2, a3, a4, a5, a6, a7), omit = c('Country', 'Tournament')),
          paste(directory, 'results/groups/results_lucky_full_season_w_followers_10s_LOGIT_tex.txt', sep = ''), row.names = FALSE)

write.csv(stargazer(a2mod2, a2mod3, a2mod4, a2mod5, a2mod6, a2mod7,
                    se = list(a22, a23, a24, a25, a26, a27), omit = c('Country', 'Tournament')),
          paste(directory, 'results/groups/results_lucky_full_season_w_followers_20s_LOGIT_tex.txt', sep = ''), row.names = FALSE)

write.csv(stargazer(a3mod2, a3mod3, a3mod4, a3mod5, a3mod6, a3mod7,
                    se = list(a32, a33, a34, a35, a36, a37), omit = c('Country', 'Tournament')),
          paste(directory, 'results/groups/results_lucky_full_season_w_followers_30s_LOGIT_tex.txt', sep = ''), row.names = FALSE)
...
write.csv(stargazer(umod2, umod3, umod4, umod5, umod6, umod7,
                    se = list(u2, u3, u4, u5, u6, u7), omit = c('Country', 'Tournament')),
          paste(directory, 'results/us/results_lucky_full_season_w_followers_LOGIT_tex.txt', sep = ''), row.names = FALSE)

write.csv(stargazer(imod2, imod3, imod4, imod5, imod6, imod7,
                    se = list(i2, i3, i4, i5, i6, i7), omit = c('Country', 'Tournament')),
          paste(directory, 'results/italian/results_lucky_full_season_w_followers_LOGIT_tex.txt', sep = ''), row.names = FALSE)

write.csv(stargazer(fmod2, fmod3, fmod4, fmod5, fmod6, fmod7,
                    se = list(f2, f3, f4, f5, f6, f7), omit = c('Country', 'Tournament')),
          paste(directory, 'results/french/results_lucky_full_season_w_followers_LOGIT_tex.txt', sep = ''), row.names = FALSE)
...
for (n in names) {
  ...
  tmp <- subdata[which(subdata$Name == n),]
  rownames(tmp) <- 1:dim(tmp)[1]
  
  for (i in 1:dim(tmp)[1]) {
    
    if (tmp$Tournament[i] == 'US') {
      
      uss <- c(uss, n)
      uc <- c(uc, tmp$Competed[i])
      
    }
    
    if (tmp$Tournament[i] == 'French') {
