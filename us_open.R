# This script runs the statistical analyses for a paper on athletes, gender, and risk for the US Open only
...
data <- read.csv(paste(directory, 'us.csv', sep = ''), fileEncoding = 'UTF-8-BOM')
...
mod6 <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Doubles + COVID + Suspension
            + Injury + Ranking_S + Titles + Majors + US_Open, family = binomial(link = logit), data = data)

# Models 7-9 are Model 6 with alternative specifications for Majors and US_Open to serve as robustness checks

mod7 <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Doubles + COVID + Suspension
            + Injury + Ranking_S + Titles + Majors_Bi + US_Open, family = binomial(link = logit), data = data)

mod8 <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Doubles + COVID + Suspension
            + Injury + Ranking_S + Titles + Majors + US_Open_Bi, family = binomial(link = logit), data = data)

mod9 <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Doubles + COVID + Suspension
            + Injury + Ranking_S + Titles + Majors_Bi + US_Open_Bi, family = binomial(link = logit), data = data)
...
submod6 <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Doubles + COVID + Suspension
               + Injury + Ranking_S + Titles + Majors + US_Open, family = binomial(link = logit), data = subdata)

# Models 7-9 are Model 6 with alternative specifications for Majors and US_Open to serve as robustness checks

submod7 <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Doubles + COVID + Suspension
            + Injury + Ranking_S + Titles + Majors_Bi + US_Open, family = binomial(link = logit), data = subdata)

submod8 <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Doubles + COVID + Suspension
            + Injury + Ranking_S + Titles + Majors + US_Open_Bi, family = binomial(link = logit), data = subdata)

submod9 <- glm(Competed ~ factor(Gender) + Age + log(Winnings + 1) + log(Winnings_20 + 1) + Doubles + COVID + Suspension
            + Injury + Ranking_S + Titles + Majors_Bi + US_Open_Bi, family = binomial(link = logit), data = subdata)
...
write.csv(stargazer(mod1, mod2, mod3, mod4, mod5, mod6, mod7, mod8, mod9, type = 'text',
          ...
          paste(directory, 'results_all_us.txt', sep = ''), row.names = FALSE)
write.csv(stargazer(submod1, submod2, submod3, submod4, submod5, submod6, submod7, submod8, submod9, type = 'text',
          ...
          paste(directory, 'results_lucky_us.txt', sep = ''), row.names = FALSE)

write.csv(stargazer(mod1, mod2, mod3, mod4, mod5, mod6, mod7, mod8, mod9,
                    se = list(rse1, rse2, rse3, rse4, rse5, rse6, rse7, rse8, rse9)),
          paste(directory, 'results_all_us_tex.txt', sep = ''), row.names = FALSE)
write.csv(stargazer(submod1, submod2, submod3, submod4, submod5, submod6, submod7, submod8, submod9,
                    se = list(subrse1, subrse2, subrse3, subrse4, subrse5, subrse6, subrse7, subrse8, subrse9)),
          paste(directory, 'results_lucky_us_tex.txt', sep = ''), row.names = FALSE)
